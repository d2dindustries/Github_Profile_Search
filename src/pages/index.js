...
const IndexPage = () => {
	...
	function _getProfileInfo(apiFunc){
		if(state.profile.username) {
			apiFunc(state,
				(success) => { setState({ canLoad: true, profile: mergeObjects(state.profile, success) }); },
	...
	const { canLoad, page, profilePage, profile, results, totalUserCount, error } = state;
	...
	return (
	  <Layout>
	    ...
	    <div className={ PAGE_CLASS }>
		    ...
		    <ResultsContainer visible={!SHOW_PROFILE} results={ results } total={ totalUserCount } loadMore={ () => { if(canLoad) setState({ canLoad: false, page: page+1 }) } } openProfile={ (changedData) => setState(changedData) } />
		    ...
		    <ResultsContainer visible={SHOW_FOLLOWERS} results={ followers } total={ FOLLOWER_COUNT } loadMore={ () => { if(canLoad) setState({ canLoad: false, profilePage: profilePage+1 }) } }  openProfile={ (changedData) => setState(changedData) } />
		</div>
	  </Layout>
	);
}
